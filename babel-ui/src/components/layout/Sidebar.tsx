...
import { Home, Book, Loader2, Search, Edit3, Network } from 'lucide-react';
 ...
 * Only shows chapters when viewing a specific novel (not on library page).
 * Automatically highlights the current chapter based on URL params.
 */
export function Sidebar() {
  const { sidebarOpen } = useSettings();
  const { getCurrentChapter, getProgressPercentage, initNovel, currentNovelId } = useReadingProgress();
  const location = useLocation();

  // Determine if we're on a novel-specific page (reading or viewing novel details)
  const isOnNovelPage = location.pathname.includes('/library/') && !location.pathname.endsWith('/library');
  const isReadingChapter = location.pathname.includes('/chapter/');

  const novelId = currentNovelId || 'default';
  const { data: chapterListData, isLoading, error } = useChapterList(novelId);
  const [searchQuery, setSearchQuery] = useState('');

  // Extract chapter ID from URL path using location pathname
  // Handles both /chapter/:id and /library/:novelId/chapter/:id patterns
  const currentChapterIdFromUrl = (() => {
    const chapterMatch = location.pathname.match(/\/chapter\/(\d+)/);
    return chapterMatch ? Number(chapterMatch[1]) : null;
  })();

  // Use currentChapterId from store, fallback to URL param
  const activeChapterId = getCurrentChapter(novelId) || currentChapterIdFromUrl;

  // Update total chapters when chapter list loads
  useEffect(() => {
    if (chapterListData?.chapters) {
      initNovel(novelId, chapterListData.chapters.length);
    }
  }, [chapterListData, initNovel, novelId]);

  // Calculate progress
  const progressPercentage = getProgressPercentage(novelId);
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto py-6 px-4 space-y-8 custom-scrollbar">
        ...
        <div className="space-y-2">
          ...
          <Link to={currentNovelId ? `/library/${currentNovelId}/characters` : '/characters'}>
            <Button variant="ghost" className="w-full justify-start h-10 px-3 text-gray-400 hover:text-white hover:bg-white/5 font-medium transition-all group">
              <Network className="mr-3 h-4 w-4 text-gray-500 group-hover:text-purple-400 transition-colors" />
              Character Map
            </Button>
          </Link>
          <Link to="/corrections">
        ...
        {/* Section: Chapters - Only show when on a novel-specific page */}
        {(isOnNovelPage || isReadingChapter) && (
          <div className="space-y-4">
            ...
            {isLoading && (
              <div data-testid="sidebar-loader" className="flex items-center justify-center py-8 text-gray-600">
        ...
        {/* Empty state when not on a novel page */}
        {!isOnNovelPage && !isReadingChapter && (
          <div className="px-2 py-8 text-center">
            <Book className="h-8 w-8 text-gray-600 mx-auto mb-3 opacity-50" />
            <p className="text-xs text-gray-500">Select a novel to view chapters</p>
          </div>
        )}
